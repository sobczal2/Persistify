...
public static class DateTimeHelpers
{
    ...
    public static TimeSpan Median(this IEnumerable<TimeSpan> spans)
    {
        ...
        if (ordered.Count % 2 == 0)
            return TimeSpan.FromTicks(
                (ordered[ordered.Count / 2 - 1].Ticks + ordered[ordered.Count / 2].Ticks) / 2
            );
    ...
    public static TimeSpan StandardDeviation(this IEnumerable<TimeSpan> spans)
    {
        ...
        var sumOfSquaresOfDifferences = spans
            .Select(s => (s - average).TotalSeconds * (s - average).TotalSeconds)
            .Sum();
