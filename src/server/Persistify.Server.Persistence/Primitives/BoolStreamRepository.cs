...
public class BoolStreamRepository : IValueTypeStreamRepository<bool>, IDisposable
{
    ...
    {
        return useLock ? await _semaphoreSlim.WrapAsync(() => ReadRangeAsync(take, skip)) : await ReadRangeAsync(take, skip);
    }
    ...
    {
        return useLock ? await _semaphoreSlim.WrapAsync(() => DeleteInternalAsync(key)) : await DeleteInternalAsync(key);
    }
